...
class QuickActionAdapter(val context: Context,var selected: Int,val cats: List<Car>,val onItemSelect:(position: Int,car: Car)->Unit) :
    ...
    override fun onBindViewHolder(holder: QuickViewHolder, position: Int) {
        ...
        if (cats[position].type=="goods"){
            ...
            holder.mExtra!!.text =cats[position].numPersons.toString()+" "+context.getString(R.string.persons)
        }
