package com.aait.ezakbus.ui.activities.map
...
import com.aait.ezakbus.ui.adapters.NearestPlacesAdapter
import com.aait.ezakbus.ui.adapters.SavedAdapter
import com.aait.ezakbus.ui.dialogs.DataDialog
import com.aait.ezakbus.ui.fragments.bottom_nav.HomeFragment
...
class SearchPlacesActivity : ParentActivity() {
    ...
    private fun sendRequestUnsave(
        ...
        repo.
            savePlace(lat.toString(),lng.toString(),name,address!!, placeId!!,mPrefs!!.token!!)
    ...
    private fun setAutoComplete() {
        ...
        if (!Places.isInitialized()) {
            Places.
                initialize(applicationContext, mPrefs!!.user!!.googlekey.toString())
        }
...
// Set up a PlaceSelectionListener to handle the response.
    ...
    private fun performSearch(key: String) {
            addDisposable(repo
                .searchPlaces(lat.toString(),lng.toString(),key,mPrefs!!.token!!)
               // .delay(1000,TimeUnit.MILLISECONDS)
                .subscribeWithLoading({
                    onStartLoad()
                },{
                    if (it.value=="1"){
                        if (it.data!!.places.isEmpty()){
                            //show no results
                            ...
                            rec_search_results.visibility=View.VISIBLE
                            progress_wheel.visibility=View.GONE
                            mSearchAdapter.swapData(it.data!!.places,true)
                        }
                    }
                    else{
